...
// Main variables
var startTime, endTime, timestamps, idx, marketTkns, dsoTkns, playerTkns, refereeTkns, marketResult, marketState, powerPeak, calcDsoTkns, calcPlayerTkns;
...
contract('MarketsManager', function([owner, dso, player, referee, cheater]) {
    ...
    describe('Successful settlements:', function() {
        ...
        it('PowerPeak <= LowerMaximum: The player takes all the DSO tokens', async function() {
            ...
            playerTkns.should.be.bignumber.equal(constants.PLAYER_TOKENS+constants.DSO_STAKING);
            ...
            marketResult.should.be.bignumber.equal(constants.RESULT_PRIZE);
            marketState.should.be.bignumber.equal(constants.STATE_CLOSED);
        });
        ...
        it('LowerMaximum < PowerPeak <= UpperMaximum: The player takes a part of the DSO tokens (revenue)', async function() {
            ...
            marketResult = await this.marketsManager.getResult(idx);
            marketState = await this.marketsManager.getState(idx);
            marketResult.should.be.bignumber.equal(constants.RESULT_REVENUE);
            marketState.should.be.bignumber.equal(constants.STATE_CLOSED);
        });
        ...
        it('PowerPeak > UpperMaximum AND the player staking is enough: The DSO takes a part of the player tokens (penalty)', async function() {
            ...
            marketResult = await this.marketsManager.getResult(idx);
            marketState = await this.marketsManager.getState(idx);
            marketResult.should.be.bignumber.equal(constants.RESULT_PENALTY);
            marketState.should.be.bignumber.equal(constants.STATE_CLOSED);
        });
        ...
        it('PowerPeak > UpperMaximum AND the player staking is not enough: The DSO takes all the player tokens (crash)', async function() {
            ...
            marketTkns.should.be.bignumber.equal(0);
            dsoTkns.should.be.bignumber.equal(constants.DSO_TOKENS + constants.PLAYER_STAKING);
            playerTkns.should.be.bignumber.equal(constants.PLAYER_TOKENS - constants.PLAYER_STAKING);

            // Check market result and state
            marketResult = await this.marketsManager.getResult(idx);
            marketState = await this.marketsManager.getState(idx);
            marketResult.should.be.bignumber.equal(constants.RESULT_CRASH);
            marketState.should.be.bignumber.equal(constants.STATE_CLOSED);
        });
    });
});
